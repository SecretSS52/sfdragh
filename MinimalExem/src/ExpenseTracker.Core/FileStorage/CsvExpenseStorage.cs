...
/// <summary>
/// Файловое хранилище расходов в формате CSV.
/// </summary>
public sealed class CsvExpenseStorage : IExpenseStorage
{
    ...
    /// <summary>
    /// Сохраняет расходы в CSV-файл с разделителем точка с запятой.
    /// </summary>
    public void Save(string path, IReadOnlyCollection<Expense> expenses)
    {
        ...
        DebugHelper.WriteTrace($"CSV сохранен. Записей: {expenses.Count}.");
    }
    ...
    {
        DebugHelper.WriteDebug($"Загрузка CSV: {path}");
        ...
        DebugHelper.WriteTrace($"CSV загружен. Записей: {result.Count}.");
        return result;
    }

    private static string Escape(string value)
    {
        // Если в тексте есть разделитель или кавычки, значение нужно обернуть в кавычки.
    ...
    {
        // Мини-парсер нужен, чтобы корректно прочитать значения в кавычках.
